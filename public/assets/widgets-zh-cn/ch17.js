...
(function () {
  ...
  var AXES = [
    {
      ...
      frags: [
        ...
        '您好，連續一週處于高壓狀態，確实值得正視。这裡整理了幾个可行的做法。',
    ...
    },
    {
      ...
      frags: [
        ...
        '可以先把讓你緊繃的事情全部列出来，分成『我能控制』和『我控制不了』兩類：能控制的，挑影响最大的一件先动手；控制不了的，練習暫时放下。另外，睡眠和短暫的運动對緩解压力很有效——哪怕只是下班后散步十分钟，也能幫大腦切換狀態。',
  ...
  var NOTE_TEXT = '玩具模型说明：真实系統中，v 是用對比激活分析，從「表現該特质 vs. 不表現該特质」' +
    '兩組回應的激活值差抽取出的殘差流方向（本章 17.1.1），并在每个詞元生成步驟施加 h ← h + αv；' +
    '这裡以預先撰寫的文字片段模擬其效果。三个滑桿各對應一个近似正交的方向，' +
    '組合方式正如複合向量 v_composite = Σ αᵢ·vᵢ——同一組權重，部署后即可調出不同个性。';
  ...
  window.ChapterWidget = {
    ...
    intro: '把三个近似正交的 persona 向量——正式↔親暱、简洁↔詳盡、直接建議↔同理傾听——' +
      ...
      '對同一句「工作壓力好大」的回覆风格隨之連續变化，体感 17.1.1 節 h ← h + αv ' +
