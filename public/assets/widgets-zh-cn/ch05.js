...
(function () {
  ...
  function buildTab1() {
    ...
    const sRc = makeSlider('被选回覆分数 r<sub>c</sub>', -5, 5, 0.1, st.rc, (v) => { st.rc = v; update(); });
    const sRr = makeSlider('被拒回覆分数 r<sub>r</sub>', -5, 5, 0.1, st.rr, (v) => { st.rr = v; update(); });
    ...
    const stD = statBox('分差 Δ = r_c − r_r'), stP = statBox('偏好机率 P = σ(Δ)'), stL = statBox('NLL 損失 −log σ(Δ − m)');
    ...
    function update() {
      ...
      let msg;
      if (Math.abs(dz) < 0.15) msg = '分差 ≈ 0：模型完全无法區分兩个回覆，P ≈ 0.5，損失 ≈ log 2 ≈ 0.693——隨机猜測的水準，也是奖励头剛初始化、尚未训练时的典型狀態。';
      else if (dz >= 4) msg = '分差很大：P = ' + fmt(P) + '，sigmoid 已飽和，損失趨近 0。这筆样本幾乎不再产生梯度，繼續拉大分差的边際效益極小。';
      else if (dz >= 1) msg = '模型已明確偏好被选回覆（P = ' + fmt(P, 2) + '），損失降至 ' + fmt(L) + '。−log σ(Δ) 的梯度隨 Δ 增大而衰減，之后的更新会越来越溫和。';
      else if (dz > 0) msg = '模型略偏好被选回覆，但信心不足：P 只比 0.5 高一點，損失仍接近 0.693，梯度会持續推动 r_c 上升、r_r 下降以扩大分差。';
      else if (dz > -4) msg = '排序錯了：模型給被拒回覆更高分（P = ' + fmt(P, 2) + ' < 0.5），損失 ' + fmt(L) + ' 高于 0.693，而且錯得越多梯度越大——这筆样本会被強力修正。';
      else msg = '嚴重排反：σ(Δ) ≈ 0，−log σ(Δ) 近似线性成长（≈ |Δ| = ' + fmt(-dz, 1) + '），这筆样本会主导整个批次的梯度。';
      if (st.m > 0) msg += ' 目前打开边際 m = ' + fmt(st.m, 1) + '：損失改以 σ(Δ − m) 計算（式 19），chosen 至少要高出 ' + fmt(st.m, 1) + ' 分損失才会壓低——標註的偏好強度越大，要求的分差就越大。';
  ...
  // ================= 分页 2：训练奖励模型 =================
  const PAIRS = [
    { q: '什么是奖励模型？', c: '奖励模型讀入提示与回覆，輸出一个純量分数，作为 RLHF 下游优化的代理訊號。', r: '奖励模型就是一種会給奖励的模型。', qc: 2.5, qr: -1.5 },
  ...
  function buildTab2() {
    ...
    const eqNote = el('p', { style: 'margin:.3rem 0 0; font-size:.85rem; color:var(--fg-muted);',
      text: '每一對回覆各有「隐藏真实品质」q。標註者依 Bradley-Terry 模型行事：把品质較高者选为 chosen 的比例为 p* = σ(q_c − q_r)。上式即 BT 的 NLL 損失對 p* 取期望后的解析梯度更新（η = ' + LR + '）。' });
    ...
    const stStep = statBox('训练步数'), stLoss = statBox('平均 BT 損失'), stFloor = statBox('雜訊下限（標註不一致）');
    ...
    legend.appendChild(el('span', { text: '┄ 收斂目標：該對平均值 ± (q_c − q_r)/2（點击任一對看内容）' }));
    ...
    loss.appendChild(svgText({ x: (LP.l + W - LP.r) / 2, y: LH - 3, 'text-anchor': 'middle', 'font-size': 11, fill: 'var(--fg-muted)' }, '训练步数 → 平均損失'));
    ...
    btnShift.addEventListener('click', () => {
      ...
      st.note = '剛才把 12 个分数全部 +1：所有分差 Δ 不变，偏好机率与損失也「完全不变」——BT 奖励沒有絕對零點，只有差值有意義。';
      update();
    });
    ...
    function update() {
      ...
      detail.innerHTML = '<strong>第 ' + (i + 1) + ' 對</strong>｜提示：「' + p.q + '」<br>' +
        ...
        '<span style="color:var(--fg-muted);">隐藏品质差 q_c − q_r = ' + fmt(p.gap, 1) + ' → 標註一致率 p* = ' + fmt(p.pstar, 3) +
        '；目前學到 Δ = ' + fmt(dz, 2) + '（收斂时 Δ → ' + fmt(p.gap, 1) + '），該對損失 = ' + fmt(pairLoss(i)) + '。</span>';
      let msg;
      if (st.step === 0) msg = '尚未训练：12 个 reward 全为 0，每一對的 P 都是 0.5，平均損失 = log 2 ≈ 0.693。按「训练一步」看看解析梯度怎么推动分数。';
      else if (L - FLOOR < 0.02) msg = '已貼近雜訊下限（虛线）：損失降不下去了，因为偏好標註本身就不一致（p* < 1）。此时每一對學到的分差 Δ ≈ 真实品质差 q_c − q_r，但「絕對分数」停在任意位置——每一對的平均值從训练开始就沒动过。';
      else if (L - FLOOR < 0.1) msg = '接近收斂：品质差大的對（如第 1 對，p* ≈ 0.98）分差拉得又快又开；品质差小的對（如第 4 對，p* ≈ 0.62）梯度早早变小、分差維持小——BT 損失自动把「標註有多一致」轉譯成「分差該多大」。';
      ...
      interp.innerHTML = '<strong>解讀：</strong>' + msg +
        ...
        '<br><strong>教學重點：</strong>奖励分数是從成對比較中「湧現」的相對量：對每个分数加同一常数 c，σ(Δ) 与損失完全不变（式 13）——所以單一分数沒有絕對意義，跨模型、甚至跨提示的分数都不能直接比大小。';
    }
    update();
  ...
  window.ChapterWidget = {
    ...
    intro: '分页 1：拖动被选／被拒回覆的分数，看 sigmoid 如何把分差变成偏好机率与 NLL 損失；分页 2：在 6 筆繁中偏好對上用解析梯度实際训练 12 个純量 reward，觀察分数如何從比較中「湧現」。',
    render(root) {
      const panels = [buildTab1(), buildTab2()];
      const tabBtns = ['1｜偏好机率', '2｜训练奖励模型'].map((t, i) => {
