/* 附录 B 交互元件：话多的平衡（Chattiness Balance）体验器 */
(function () {
  'use strict';
  ...
  var TEXTS = [
    'RLHF（基于人類回饋的强化学习）是一種利用人類偏好訊號来微调语言模型的训练方法。',

    'RLHF 是「基于人類回饋的强化学习」：先讓标注者比較模型的多个回复并选出較好的那个，' +
    '用这些偏好数据训练一个獎勵模型，再以强化学习讓语言模型往人類偏好的方向更新。' +
    'ChatGPT 等聊天助手「好聊」的特质，多半来自这一步。',

    '基于人類回饋的强化学习（RLHF）是机器學習的一个子領域，目标是把人類的判斷納入模型的训练迴圈。' +
    '流程通常分三步：先以監督式微调讓模型學会遵循指令；接着收集人類對成對回复的偏好比較，' +
    '训练出一个獎勵模型来近似人類的喜好；最后用 PPO 或 DPO 等算法，' +
    '讓语言模型在不偏離原模型太远的前提下最大化这个獎勵。\n\n' +
    '相較于手工设计獎勵函数，这種做法能捕捉「什么样的回答比較好」这類难以明文定義的主觀標準，' +
    '但也面臨标注成本高、回饋不一致与獎勵駭客（reward hacking）等挑战。',

    '基于人類回饋的强化学习（**RLHF**）是强化学习的一个子集，其中引导學習的獎勵由人類提供，' +
    '而非預先指定的数值獎勵函数。这在难以设计獎勵函数、或期望的表現涉及主觀判斷时特別有用。\n\n' +
    '**核心組成：**\n\n' +
    '1. **人類输入：** 回饋来自人類评估者的定性判斷，形式包含数值評分、二元偏好、自然語言回饋或示范。\n' +
    '2. **獎勵模型：** 原始人類回饋存在变异与偏差，需先训练獎勵模型，把偏好转换成穩定的學習訊號。\n' +
    '3. **策略优化：** 以 PPO、DPO 等算法迭代更新模型，使其行为逐步貼近人類偏好。\n\n' +
    '**應用領域：**\n\n' +
    '- **对话助手：** 讓回复更有幫助、无害且誠实。\n' +
    '- **机器人學：** 教机器人执行难以用獎勵函数描述的任务。\n' +
    '- **推薦系統：** 用人類回饋调整并改善推薦品质。\n\n' +
    '**主要挑战：**\n\n' +
    '1. **可擴展性：** 收集人類回饋既耗时又昂貴。\n' +
    '2. **偏差与雜訊：** 人類回饋不一致，可能导致次佳的學習结果。\n' +
    '3. **对齐与獎勵駭客：** 模型可能只學会討好表面訊號，而非真正的人類偏好。',

    '这是一个非常好的问題！基于人類回饋的强化学习（RLHF，Reinforcement Learning from Human ' +
    'Feedback）是一个内容相当丰富的主题，以下我將盡可能完整地为您说明。\n\n' +
    '首先，需要说明的是，RLHF 是一種利用人類偏好来训练语言模型的方法。換句話说，' +
    '它讓人類的回饋參与模型的學習过程。也就是说，模型的训练訊號来自人類的判斷，' +
    '而不是預先定義的獎勵函数。\n\n' +
    '**詳細流程如下：**\n\n' +
    ...
    '因此数据品质非常关鍵，务必重視数据品质。\n' +
    '3. **獎勵模型训练：** 然后训练獎勵模型。如前所述，獎勵模型的作用是把人類偏好转换成训练訊號，' +
    '这正是第 2 步收集偏好数据的目的所在。\n' +
    '4. **强化学习：** 最后进行强化学习优化。这一步会用到前面所有步驟的成果，' +
    '也就是说，它整合了前述的一切。\n\n' +
    '**不过，需要注意的是**，实際效果可能因模型规模、数据品质与具体應用情境而异，以上内容僅供參考，' +
    '不構成任何工程決策建議。在实際部署前，建議諮詢相关領域的专业人士，并进行充分的评估与测试。' +
    '同时也要提醒您，AI 領域发展迅速，本说明可能无法涵盖最新的进展。\n\n' +
    '总結来说，RLHF 就是用人類回饋来训练模型的方法——正如开头所说，' +
  ...
  var DPO_TEXT =
    '基于人類回饋的强化学习（**RLHF**）是讓语言模型对齐人類偏好的核心技术：' +
    '引导學習的獎勵訊號来自人類判斷，而非預先寫定的獎勵函数。\n\n' +
    '典型流程包含三个阶段：\n\n' +
    '1. **監督式微调（SFT）：** 以人工示范数据训练模型遵循指令，作为后續训练的起點。\n' +
    '2. **獎勵模型训练：** 收集标注者對成對回复的偏好比較（被选 ≻ 被拒），' +
    '训练一个能为任意回复打分的獎勵模型。\n' +
    '3. **强化学习优化：** 以 PPO 或 DPO 等算法更新策略，在最大化獎勵的同时，' +
    '以 KL 懲罰約束模型不要偏離參考模型太远。\n\n' +
    '**为什么重要：** 这種做法能捕捉「什么样的回答比較好」这類难以明文定義的主觀標準，' +
    '是 ChatGPT 等助手好用的关鍵。\n\n' +
    '**主要挑战：** 标注成本高、人類回饋充滿雜訊与偏差，' +
    '以及模型可能學会鑽獎勵的漏洞（獎勵駭客）。';

  var INTERPS = [
    '一句話回答的信息密度最高，但在成對比較中，評審常覺得它「不夠周全」——精简的答案很容易輸給看起来更完整的對手。',
    '简短但有内容。Starling Beta 这類模型示范了「长得剛好」：平均响应变长了，但增加的方式確实幫助到人類評分者。',
    '接近甜蜜點。評審（人類与 LLM）平均偏好較长、較完整的回复——这是 RLHF 模型越来越囉唆的原因之一。試試切換上方的 SFT／DPO 對照。',
    '偏好微调的典型风格：粗体、編號、條列清單。勝率仍高，但每个字承載的信息已开始稀釋——「话多」不只是长度，也包含格式編排。',
  ...
  /* 曲线：人類偏好勝率（倒 U，峰值中间偏右）与信息密度（單調下降） */
  ...
  function render(rootEl) {
    ...
    var lab = el('label', '', row); lab.textContent = '回复詳盡度';
    ...
    var s = svg('svg', { viewBox: '0 0 560 232', role: 'img', 'aria-label': '回复长度与偏好勝率、信息密度的关係图' }, chart);
    /* 甜蜜點區带 */
  ...
  window.ChapterWidget = {
    title: '话多的平衡体验器',
    intro: '拖动「回复詳盡度」滑桿，觀察同一个提问在五種詳盡度下的示范回复，并對照曲线图上的人類偏好勝率（倒 U 型）与信息密度（單調下降）；再切換 SFT／DPO 模型，体会偏好微调如何讓回复变得更长、更結構化。',
    render: render
  };
})();
