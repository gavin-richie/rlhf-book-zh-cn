...
(function () {
  ...
  function buildPPOTab() {
    ...
    ctrl.appendChild(sliderRow('裁剪係数 ε', 0.05, 0.5, 0.01, st.eps, function (v) { st.eps = v; update(); }).row);
    ctrl.appendChild(sliderRow('优勢 A', -2, 2, 0.05, st.A, function (v) { st.A = v; update(); }).row);
    ...
    panel.appendChild(el('p', 'margin:.25rem 0 0;font-size:.78rem;color:var(--fg-muted);', '在图上按住并拖曳，即可移动 ρ 游標點。'));
  ...
  function buildGRPOTab() {
    ...
    function update() {
      ...
      svgText(svg, 13, PAD.t + plotH / 2, 'Â（群組优勢）', { 'text-anchor': 'middle', 'font-size': 12, fill: 'var(--fg)', transform: 'rotate(-90 13 ' + (PAD.t + plotH / 2) + ')' });
      ...
      if (degenerate) {
        ...
        interp.textContent = '警示：全對或全錯时 std → 0，群組优勢全为 0——这一批样本裡沒有任何對比，GRPO 在这批样本上學不到东西。这正是課程设计（curriculum）重要的原因：題目难度必須落在模型「有时對、有时錯」的區间，群組内才有可供比較的學習訊號。';
      } else {
