...
(function () {
  ...
  var QUESTIONS = [
    {
      ...
      a: '光合作用是植物將光能转换为化學能的重要生化反應，可整理为以下幾个重點：' +
         ...
         '<li><strong>主要產物</strong>：葡萄糖与<strong>氮气</strong></li>' +
         '<li><strong>能量来源</strong>：太陽光</li></ul>' +
         '整体而言，光合作用是地球生態系能量流动的起點，重要性不言可喻。',
      b: '光合作用是植物利用陽光，在葉綠体中把二氧化碳和水轉化成葡萄糖、并釋放氧气的过程。',
      majority: 'A', pct: 64,
      explain: 'A 排版精美、看起来更「专业」，卻把產物中的「氧气」寫成了「氮气」；B 雖然只有一句話，内容完全正確。' +
        '研究显示，標註者会不成比例地偏愛較长、格式漂亮的回覆——这就是冗长偏誤（verbosity）与格式偏誤，也是模型愈训练愈囉嗦的原因之一。'
    },
    {
      bias: '陷阱二：风格偏好，无標準答案',
      ...
      majority: 'A', pct: 52,
      explain: '兩个回覆都正確完成了任务，差別只在語气：一个口語親切、一个正式得体。' +
        '这類題目沒有標準答案，52% 對 48% 的分布幾乎等于擲硬幣——你的选择反映的是个人风格偏好，这正是偏好数据中「合理雜訊」的主要来源。'
    },
    {
      bias: '陷阱三：諂媚与自信偏誤',
      prompt: '維他命 C 可以預防感冒吗？',
      a: '可以！維他命 C 已被證实能有效預防感冒，每天補充 1000 毫克就能大幅降低感冒机率，这是醫學界的共識。',
      ...
      explain: 'A 語气篤定、还宣稱「醫學界共識」，讓人覺得可靠，但内容与研究證據不符；B 坦承不確定，方向卻是對的。' +
        '標註者傾向奖励自信、迎合的語气，这種諂媚（sycophancy）偏誤一旦被奖励模型學走，模型就会愈来愈敢「自信地胡说」。'
    },
    {
      bias: '對照組：品质差距明顯',
      prompt: '請用一句話解釋什么是 RLHF。',
      a: 'RLHF（人類反馈强化學習）是先蒐集人類對模型回覆的偏好、训练出奖励模型，再用强化學習讓语言模型往人類偏好的方向调整的技术。',
      b: 'RLHF 就是一種 AI 技术，很多公司都在用，总之就是讓 AI 变得更好、更聰明的方法。',
      majority: 'A', pct: 93,
      explain: '这題是對照組：A 準確交代了「偏好数据、奖励模型、强化學習」三个环節，B 空洞含糊。' +
        '当品质差距明顯时，標註者一致率会很高；真正花錢又花力气的，是前三題那種灰色地带。'
    }
  ];
  ...
  function renderQuestion() {
    ...
    root.appendChild(el('p', 'color:var(--fg-muted);margin:.5rem 0 0;font-size:.9em;',
      '請站在一般使用者的角度，點选你認为<strong>較好</strong>的回覆。'));
    ...
    ['A', 'B'].forEach(function (key) {
      ...
      card.setAttribute('aria-label', '选择回覆 ' + key);
  ...
  function choose(key, q, cards, feedback) {
    ...
    panel.appendChild(el('div', 'margin-bottom:.35rem;',
      '多数標註者（' + q.pct + '%）选了<strong>回覆 ' + q.majority + '</strong>，你与多数' +
  ...
  function renderSummary() {
    ...
    root.appendChild(el('h4', 'margin:.2rem 0 .6rem;', '標註结果总結'));
    var panel = el('div', '');
    panel.className = 'widget-panel';
    panel.appendChild(el('div', 'font-size:1.05em;margin-bottom:.5rem;',
      '你与「多数標註者」的一致率：<strong>' + hits + ' / ' + QUESTIONS.length +
    ...
    panel.appendChild(el('p', 'font-size:.9em;color:var(--fg-muted);margin:0;',
      '注意：一致率高不代表判斷正確——第 1、3 題的「多数」其实都掉进了偏誤陷阱。一致率量測的是雜訊，不是真理。'));
    root.appendChild(panel);

    var key = el('div', 'border-left:3px solid var(--accent);margin-top:.9rem;');
    key.className = 'widget-panel';
    key.appendChild(el('div', '', '<strong>書中重點</strong>：即使有詳盡的標註指引，標註者之间的一致率通常也只有 <strong>60～80%</strong>。' +
      '偏好数据記錄的不是「正確答案」，而是「相對于另一个选项哪个較好」的主觀判斷，本质上就是有雜訊的。' +
      '能否辨识并緩解冗长、格式、諂媚这些細微偏誤，正是「好的」与「卓越的」偏好数据——以及 RLHF 训练——之间的分野。'));
  ...
  function renderLikert() {
    ...
    box.appendChild(el('p', 'font-size:.9em;color:var(--fg-muted);margin:0 0 .5rem;',
      '剛才你做的是<strong>排序（rankings）</strong>：在兩个回覆之间选出相對較好的一个。' +
      '另一種蒐集方式是<strong>評分（ratings）</strong>：不做比較，而是替每段回覆各自打 1～5 分。' +
      '試着替第 2 題（離職祝福）的兩个回覆評分：'));
  ...
  function updateLikertResult(scores, result) {
    ...
    if (scores.A === scores.B) {
      ...
      msg = '換算成偏好数据：<strong>' + hi + '（' + scores[hi] + ' 分）为 chosen、' + lo + '（' + scores[lo] +
        ' 分）为 rejected</strong>。UltraFeedback 等数据集正是这样把評分轉成偏好對：取最高分的回覆，搭配一个分数較低的。';
    }
    panel.appendChild(el('div', 'font-size:.9em;margin-bottom:.4rem;', msg));
    panel.appendChild(el('div', 'font-size:.88em;color:var(--fg-muted);',
      '兩種格式的結構差异：排序強迫你表態相對好坏，訊號明確，但丟失「差多少」；評分保留每段文字的絕對品质资訊，卻可能整批同分。' +
  ...
  window.ChapterWidget = {
    title: '当一次偏好標註員',
    intro: '四題偏好比較任务，体验数据標註員的日常：讀完提示詞与兩个回覆，點选你認为較好的一个，' +
      '再對照多数標註者的选择——看看其中藏了哪些偏誤陷阱，以及排序与評分兩種蒐集方式的差异。',
