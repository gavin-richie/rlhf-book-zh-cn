...
const RAW_DIR = join(process.cwd(), "src/data/chapters-raw");

// Cache for bibliography entries (loaded on first access)
let bibCache: { html: string; entries: Map<number, string> } | null = null;

// Clean PDF copy-artifacts in bibliography markdown: strip page numbers /
// site markers and form-feeds, then split merged bracket citations and
// convert `[N] Author...` to `N. Author...` so the numbered list parser
// emits an `<li>` for every entry (including the 21 that get concatenated
// onto the previous entry's line by the PDF).
...
/**
 * Inject `id="ref-N"` anchors on bibliography <li> elements.
 * Only called on the bibliography chapter, so a simple counter is safe.
 */
...
function preprocessMath(md: string): string {
  // Wrap standalone \begin{aligned}...\end{aligned} blocks in $$ delimiters.
  // The original markdown has aligned blocks on their own lines (not inside $$).
  // Handle cases where \end{aligned} is followed by \tag{...} on the same line.
  return md.replace(
...
export async function getChapterContent(id: string): Promise<{ html: string; toc: { id: string; text: string }[]; meta: ChapterMeta }> {
  ...
  // Bibliography chapter: apply the same PDF-cleanup that loadBibliography
  // does internally, so all 409 numbered entries become `<li>` items. Skip
  // citation wrapping — the bibliography's own `[N]` list markers must
  // remain as line-start bracketed numbers, otherwise wrapCitations would
  // inline them as `<a>` tags and break the numbered list parsing.
  if (id === "bibliography") {
    ...
    try {
      ...
      // bibliography not found or error — silently skip citation wrapping
    }
  }
  // Wrap \begin{aligned} blocks in $$ delimiters so katex can render them
  ...
  // Add ref-N anchors on bibliography entries
  if (id === "bibliography") {
